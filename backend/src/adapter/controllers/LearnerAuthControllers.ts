...
import LearnerLogoutUseCase from "../../application/use_cases/learner/LearnerLogoutUseCase";
...
import ChangePasswordLearnerUseCase from "../../application/use_cases/learner/ChangeLearnerPasswordUseCase";
...
const learnerLogoutUseCase = new LearnerLogoutUseCase(learnerRepository);
...
class LearnerAuthController {
  ...
  async learnerLogout(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const learnerId = req.user?.userId || "";
      const response = await learnerLogoutUseCase.execute(learnerId);
      if (response.success && response.data) {
